...
public class PipelineClient implements ResourceClient<V1alpha1Pipeline> {
    ...
    public boolean update(V1alpha1Pipeline oldPipeline, V1alpha1Pipeline newPipeline) {
        ...
        try {
            ...
            logger.warn(String.format("Unable to patch Pipeline '%s/%s', reason: %s, body: %s",
    ...
    public V1alpha1Pipeline create(V1alpha1Pipeline pipeline) {
        DevopsAlaudaIoV1alpha1Api api = new DevopsAlaudaIoV1alpha1Api();
        try {
            return api.createNamespacedPipeline(pipeline.getMetadata().getNamespace(), pipeline, null, null, null);
        } catch (ApiException e) {
            logger.warn(String.format("Unable to create Pipeline '%s/%s', reason: %s", pipeline.getMetadata().getNamespace(), pipeline.getMetadata().getName(), e.getMessage()), e);
            return null;
        }
    }
