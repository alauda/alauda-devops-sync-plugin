...
public class Clients {
  private static ConcurrentHashMap<Class, ResourceClient> registeredClients =
  ...
  public static boolean registeredResourceSynced(Class... classes) {
    return Clients.getRegisteredClients()
        ...
        .stream()
        .filter(
            classResourceClientEntry ->
                Arrays.asList(classes).contains(classResourceClientEntry.getKey()))
        .allMatch(
            classResourceClientEntry -> classResourceClientEntry.getValue().informer().hasSynced());
  }
}
